...
generateKids <- function(lambda, kappa, parent) {
  ...
  n.kids = rpois(lambda=lambda * parent.life, n=1)
    ...
    if (n.kids) {
        ...
        # for each child
      kid.birthdays = parent$birth.date + runif(n=n.kids, min=0, max=parent.life)
        # <life.lengths>: a random numeric vector indicating the length of
        # each child's life
      kid.ass = parent$assassination.date + rexp(rate=kappa, n=n.kids)
      
      kids = data.frame(parent.id=parent$child.id, child.id=1:n.kids, birth.date=kid.birthdays, assassination.date=kid.ass)
...
nextGeneration <- function(lambda, kappa, parents) {
  ...
  next.gen = by(parents, seq_len(nrow(parents)), function(parent) generateKids(lambda=lambda, kappa=kappa, parent=parent))
  ...
  if(!is.null(next.gen)) {
    ...
    rownames(next.gen) = 1:nrow(next.gen)
    next.gen
  } else {
    NULL    
  }
...
bAGen <- function(lambda, kappa, max.gen) {
    ...
    # your code here
    ancestry = data.frame(parent.id=0, child.id=0, birth.date=0, assassination.date=rexp(rate=kappa, n=1))
    generation.list[[1]] = ancestry
    ...
    for (i in 1:(max.gen-1)) {
      if (is.null(generation.list[[i]])) {
        break
      }
      
      generation.list[[i+1]] = nextGeneration(lambda, kappa, generation.list[[i]])
    }
...
sim.2 = replicate(1000, bAGen(0.1, 0.50, 10)) 
...
gensSurvived <- function(bagen.simulation) {
    ...
    # your code here
  unlist(lapply(bagen.simulation, length))
...
# your code hereas.vecto
plot(density(gensSurvived(sim.1), bw=0.35), col="red", main="B-A Simulation", xlab="generations survived", xlim=c(0,11))
lines(density(gensSurvived(sim.2), bw=0.35), col="green")
lines(density(gensSurvived(sim.3), bw=0.35), col="blue")
legend("topright", c("sim.1", "sim.2", "sim.3"), col=c("red", "green", "blue"), lty=1)
